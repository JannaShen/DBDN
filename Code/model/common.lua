...
function downsample_wo_act(scale, method, nFeat)
    local scale = scale or 2
    local method = method or 'espcnn'
    local nFeat = nFeat or 64
    ...
    if method == 'deconv' then
        if scale == 2 then
            return conv(nFeat,nFeat, 6,6, 2,2, 2,2)
        elseif scale == 3 then
            return conv(nFeat,nFeat, 9,9, 3,3, 3,3)
        elseif scale == 4 then
            return seq()
                :add(conv(nFeat,nFeat, 6,6, 2,2, 2,2))
                :add(conv(nFeat,nFeat, 6,6, 2,2, 2,2))
        elseif scale == 8 then
            return seq()
                :add(conv(nFeat,nFeat, 6,6, 2,2, 2,2))
