...
path='/data/jishen/SR/DBDN/code/Prepare_TrainData/Flickr2K/Flickr2K_HR'
ext               =  {'*.jpg','*.png','*.bmp'};
filepaths           =  [];
for i = 1 : length(ext)
    filepaths = cat(1,filepaths, dir(fullfile(path, ext{i})));
end
...
for idx_im = 1:nb_im
    fprintf('Read HR :%d\n', idx_im);
    ImHR = imread(fullfile(path, filepaths(idx_im).name));
    Flickr2K_HR{idx_im} = ImHR;
...
for IdxIm = 1:nb_im
    ...
    FolderLRx2 = fullfile(path_save, 'Flickr2K_LR_bicubic', 'X2');
    FolderLRx3 = fullfile(path_save, 'Flickr2K_LR_bicubic', 'X3');
    FolderLRx4 = fullfile(path_save, 'Flickr2K_LR_bicubic', 'X4');
